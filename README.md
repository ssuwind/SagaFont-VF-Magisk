...
1.0.x 使用单一静态字重，系统字重调节无效。本版本内置**可变字体** `Shajia-VF.ttf`（wght 轴 100~900，默认 350 偏细显示），通过 `fonts.xml` 的 `<axis tag="wght">` 声明注册可变字重族：

- 在 **ColorOS 16 系统设置 → 字体粗细** 中拖动滑块，可无级驱动沙加体的 wght 轴（100→16px、350→29px、400→32px、900→59px 笔画厚度单调递增）。
